...
import {
  formatFlagCondition,
  formatFlagEnvironmentOutcome,
  formatFlagEnvironmentVariantSummary,
  formatFlagRolloutOutcome,
  ...
  for (const [envName, envConfig] of sortedEnvs) {
    ...
    if (envConfig.active) {
      ...
      const envSummary = hasCustomConfiguration
        ? 'custom'
        : formatFlagEnvironmentOutcome(envConfig.fallthrough, flag.variants);
      ...
      if (envConfig.targets && Object.keys(envConfig.targets).length > 0) {
        ...
        for (const [variantId, entityKinds] of Object.entries(
          ...
          const variantSummary = formatFlagEnvironmentVariantSummary(
            variant,
            variantId
          );
      ...
      if (envConfig.rules && envConfig.rules.length > 0) {
        ...
        for (const rule of envConfig.rules) {
          const outcome = formatFlagEnvironmentOutcome(
            rule.outcome,
            flag.variants
          );
      ...
      if (hasCustomConfiguration && envConfig.fallthrough) {
        ...
        if (fallthrough.type === 'variant') {
          ...
          const defaultSummary = formatFlagEnvironmentVariantSummary(
            defaultVariant,
          ...
          output.print(
            `      ${chalk.dim('Rollout:')} ${formatFlagRolloutOutcome(
              fallthrough,
              flag.variants
            )}\n`
          );
      ...
      const pausedSummary = formatFlagEnvironmentVariantSummary(
        pausedVariant,
